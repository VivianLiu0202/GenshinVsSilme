...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent)
{
    ...
    timer->start(50);//表示每次timeout的时间间隔是40ms
    view->show();
}
...
void MainWindow::check()
{
    ...
    if (++counter >= time)
    {
        ...
        foreach (QGraphicsItem *item, items)
        {
            ...
            {
                scene->addPixmap(QPixmap(":/images/lose.png"))->setPos(450,280);
                scene->advance();
                timer->stop();
                return;
            }
        ...
        //没输，再判断是不是最后一次，如果是的话：并且没有史莱姆了，就赢了
        if (SLIME_GEN_TIMES_NOW == SLIME_GEN_TIMES)
        {
            foreach (QGraphicsItem *item, items)
            {
            ...
            //到这里说明没有史莱姆了，赢了！！！
            scene->addPixmap(QPixmap(":/images/Victory.png"))->setPos(400, 270);
            scene->advance();
            timer->stop();
            return;
        }
